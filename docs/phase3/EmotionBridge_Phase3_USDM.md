...
| 仕様ID | 仕様グループ | 仕様 | 備考 |
|--------|-------------|------|------|
| EB3-A01-01-001 | \<除外方法\> | NVラベルの時間区間を物理的に切除し、非NV区間を連結する（デフォルト）。切除後の音声が0.1秒未満の場合は無音化にフォールバックする。設定により従来の無音化方式にも切替可能 | 無音化はeGeMAPSの区間統計量（エネルギー、F0、VUV率等）を汚染するため、切除をデフォルトとする |
...
| 仕様ID | 仕様グループ | 仕様 | 備考 |
|--------|-------------|------|------|
| EB3-B01-02-001 | \<距離関数\> | クロスドメイン整合（pooled z-score）後の特徴量空間で、V-02偏相関ベースの特徴量重みを用いた重み付きユークリッド距離を使用する（デフォルト）。重み未取得時やフォールバック設定時は等重みユークリッド距離を使用 | クロスドメイン整合によりJVNV/VOICEVOX間で「1σ」の意味を統一。重み付きにより制御不能次元（jitter/shimmer等）の影響を抑制 |
| EB3-B01-02-002 | \<推奨パラメータ\> | 各感情eについて、韻律距離が最小のk件のVOICEVOXサンプルの5D制御パラメータを推奨パラメータ候補とする | 初期運用はk=25。Phase 3bでk={15,25,50}を比較し最終決定 |
...
| EB3-C01-01-003 | \<ラベル変換\> | WRIMEの連続強度値から6クラス分類ラベルへの変換方式を定義する | 決定: argmax方式を本採用。soft labelは補助比較実験として評価 |
...
| 仕様ID | 仕様グループ | 仕様 | 備考 |
|--------|-------------|------|------|
| EB3-C01-02-001 | \<ベースモデル\> | sbintuitions/modernbert-ja-130m を使用する | 旧Phase 0と同一のベースモデル |
...
| EB3-D01-003 | \<アーキテクチャ\> | デフォルトはDeterministicMixer: `tanh(emotion_probs @ teacher_matrix)` で教師表との線形混合を行う。teacher\_matrixは6×5の感情別推奨パラメータ行列。NN版（Linear(6, 64) → ReLU → Dropout(0.3) → Linear(64, 5) → tanh）はPhase 4でテキスト埋め込み追加時に使用 | 教師データが感情ごとの固定推奨パラメータの場合、NNの最適解は本質的にこの線形混合に収束するため、中間層を排除して過学習・再現性コストを排除 |
| EB3-D01-004 | \<損失関数\> | DeterministicMixerでは学習不要（教師表から直接構築）。NN版使用時は重み付きMSEを使用する: loss = Σ\_e p(e\|text) × MSE(predicted\_params, recommended\_params\_e) | DeterministicMixerは解析的に最適であり学習ループが不要 |
...
| EB3-F01-004 | \<デフォルト動作\> | 感情分類の確信度が低い場合（全クラスの確率が均等に近い場合）、ニュートラルスタイルかつ5Dパラメータをゼロとして出力する | 初期運用値: 0.30。Phase 3d主観評価で最終微調整 |
...
| 項目 | 内容 |
|------|------|
| **要求** | 韻律距離とパラメータ予測精度の2指標で定量評価を実施する |
...
| 仕様ID | 仕様グループ | 仕様 | 備考 |
|--------|-------------|------|------|
| EB3-G01-01-001 | \<ラウンドトリップ韻律距離\> | 予測パラメータで生成した音声のeGeMAPSとJVNV感情プロファイルとの距離を計算する。目標: 必達はV-03基準（mean 3.84）比+10%以内、ストレッチは3.84未満 | |
| ~~EB3-G01-01-002~~ | ~~\<感情分類一致率\>~~ | ~~生成音声を感情分類器に通し、元のテキスト感情と一致するかを測定する~~ | **廃止**: Phase 0はテキスト感情分類器（BERT+回帰ヘッド）であり音声感情認識（SER）ではない。生成音声を入力として処理する機能がないため物理的に実行不可能。EB3-G01-01-001（韻律距離）とEB3-G01-01-003（MAE）で品質を十分に評価可能 |
...
| EB3-F01-004 | 低確信度時のニュートラルフォールバック閾値の最終微調整 | Phase 3d完了時 | プロジェクト実装者 |
| EB3-N01-002 | WRIME商用利用可否の確認 | 商用化判断時 | プロジェクト実装者 |

解消済み:
- EB3-C01-01-003（WRIME連続強度→6クラスラベル変換）はargmax本採用で確定済み
...
| 日付 | 変更内容 | 対象仕様ID |
|------|---------|-----------|
| 2026-02-19 | NV区間処理を無音化から切除（excise）に変更。無音化はeGeMAPS区間統計量を汚染するため | EB3-A01-01-001 |
| 2026-02-19 | 距離関数をクロスドメイン整合（pooled z-score）後の重み付きユークリッド距離に変更。JVNV/VOICEVOX間の正規化基準不一致と制御不能次元の影響を解消 | EB3-B01-02-001 |
| 2026-02-19 | パラメータ生成器のデフォルトアーキテクチャをDeterministicMixer（教師表線形混合）に変更。NN版はPhase 4用に保持 | EB3-D01-003, EB3-D01-004 |
| 2026-02-19 | 感情分類一致率（EB3-G01-01-002）を廃止。Phase 0はテキスト分類器であり生成音声の処理が物理的に不可能 | EB3-G01-01-002 |
