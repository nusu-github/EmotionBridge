...
ずんだもんの5スタイル（あまあま, ノーマル, セクシー, ツンツン, なみだめ）について、各感情重心からのユークリッド距離を計測したところ、**同一感情内のスタイル間距離差が極めて小さかった**。
...
**スタイル間の最大距離差は 10^-6 オーダー。** これは VOICEVOX のスタイルバリエーションが eGeMAPSv02 の88次元空間上ではほぼ同一の点に写像されることを意味する。

### 解釈（前処理依存性に注意）

この結果には前処理の影響が強い。VOICEVOX 側は `style_id` ごとの Z スコア正規化（`speaker_mode: style_id`）を行っており、理論上、各スタイル重心は原点近傍に揃う。そのため `emotion_style_distance_matrix.json` の `max_pairwise_centroid_distance` は 2.73×10^-6 と極小になった。

`build_style_mapping.py` はこの退化を検知すると raw 特徴の global z-score 空間にフォールバックする実装だが、閾値が `1e-6` なので今回は発火せず（2.73×10^-6 > 1e-6）、ほぼ退化空間で距離計算された。

raw 特徴でスタイル効果量（η²）を確認すると、平均 0.067・最大 0.354（例: F3frequency stddevNorm）であり、スタイル差が完全にゼロというわけではない。したがって:

1. **「スタイル差がない」ではなく「現行正規化パイプライン上ではスタイル差が消える」が正確**
2. 感情制御の主軸が 5D 制御パラメータである点は維持できるが、スタイル評価は raw/global z-score など別空間で再評価すべき
3. 実運用では、客観指標だけでなく聴覚評価を併用してスタイル選択を行うのが妥当
...
**スタイル間距離の微小差。** VOICEVOX コードにおいて `style_id` は `ids_for::<TalkDomain>(style_id)` のように使われており、**どのニューラルネットの重みセットを使うかを選択するキー**として機能する。`AudioQuery` にスタイル固有の音響パラメータが載るわけではなく、韻律制御パラメータ（F0・速度・エネルギー等）の解釈ロジックはスタイル間で共通である。加えて本分析では `style_id` ごとに Z スコア正規化した空間を使っているため、スタイル重心がほぼ一致しやすい。したがって 10^-6 オーダーの距離差は「実装構造」と「前処理」の両方で説明できる。
