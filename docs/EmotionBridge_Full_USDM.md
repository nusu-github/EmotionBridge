...
| EB-P0-01-01-005 | \<ラベル変換\> | WRIMEの連続強度値（0–3）を温度付き正規化して6D soft label（確率分布）を生成し、学習の教師信号として使用する | hard label（argmax）は学習データに保持しない |
| EB-P0-01-01-006 | \<データ分割\> | フィルタリング後のデータに対してstratified split（train 80% / val 10% / test 10%）を適用する。stratification keyはsoft labelのクラスタIDとする | 層化が成立しない場合はfail-fast |
...
| 仕様ID | 仕様グループ | 仕様 | 備考 |
|--------|-------------|------|------|
| EB-P0-01-02-001 | \<ベースモデル\> | sbintuitions/modernbert-ja-130m を使用する | 旧Phase 0と同一。約110Mパラメータ |
...
| 仕様ID | 仕様グループ | 仕様 | 備考 |
|--------|-------------|------|------|
| EB-P0-01-04-001 | \<評価指標\> | soft labelに対する平均KL divergence、平均cross entropy、Brier score、感情別MAEを算出する | 確率分布予測の整合性を評価 |
| EB-P0-01-04-002 | \<Go/No-Go\> | Phase 0単体の固定閾値ゲートは設けない。評価指標は `evaluation.json` に記録し、継続監視する | 品質管理はパイプライン全体で実施 |
| EB-P0-01-04-003 | \<悪化時対策\> | 指標悪化時は (1)データ品質分析、(2)温度・学習率再調整、(3)ベースモデル変更、(4)データ追加 の順で改善する | |
...
| ~~EB-P3-02-002~~ | ~~\<定量目標2\>~~ | ~~感情分類一致率: anger/happy/sadで60%以上~~ | **廃止**: Phase 0はテキスト感情分類器であり音声感情認識（SER）ではないため、生成音声を通す評価は物理的に不可能。EB-P3-02-001（韻律距離）とEB-P3-02-003（MAE）で代替 |
...
| EB-X02-005 | \<BERTモデル\> | sbintuitions/modernbert-ja-130m のライセンスに準拠 | |
