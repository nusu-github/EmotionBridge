...
| EB-P0-01-01-005 | \<ラベル変換\> | WRIMEの連続強度値（0–3）から6クラス分類ラベルへの変換方式を定義する | 暫定（T.B.D）: argmax方式、閾値方式等を比較実験で決定。Phase 3a完了時までに決定 |
...
| EB-P0-01-04-002 | \<Go/No-Go\> | マクロF1 ≥ 0.40、かつanger/happy/sadの個別F1 ≥ 0.50 を最低条件とする | 暫定（T.B.D）: 旧Phase 0のGo/No-Go基準は回帰用。分類タスク用の閾値をPhase 3a初期に確定。韻律分離が明確な3感情を重視 |
...
| 仕様ID | 仕様グループ | 仕様 | 備考 |
|--------|-------------|------|------|
| EB-P3-02-001 | \<定量目標1\> | ラウンドトリップ韻律距離がV-03のマッチング距離（mean 3.84）を下回ること | |
...
| 仕様ID | 内容 | 決定期限 | 決定者 |
|--------|------|----------|--------|
| EB-P0-01-01-005 | WRIME連続強度→6クラスラベルの変換方式 | Phase 3a完了時 | プロジェクト実装者 |
| EB-P0-01-04-002 | Phase 0分類タスク用Go/No-Go閾値の確定 | Phase 3a初期 | プロジェクト実装者 |
